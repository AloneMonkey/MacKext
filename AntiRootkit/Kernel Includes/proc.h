#ifndef inficere_proc_h
#define inficere_proc_h
...
/*
 *	Complement the proc structure
 *
 *	From osfmk/i386/locks.h
 */
struct lck_mtx_t {
	union {
		struct {
...
/*
 *	Needed to access the proc structure passed to the syscall
 *
 *	Some fields were commented out, so no other kernel includes would be needed
 *	This is valid because we are not doing any copies of this structure, just defining it to avoid dereference pointer to incomplete types
 *
 *	Made for Snow Leopard, but works on Mountain Lion (10.8.3)
 *
